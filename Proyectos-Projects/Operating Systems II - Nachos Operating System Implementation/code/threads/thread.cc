...
Thread::Thread(const char *threadName, bool joinable, int prio)
{
    ...
    this->joinLock = new Lock(threadName);
    this->isJoinable = joinable;
    this->waitingForJoin = false;
    
    if (this->isJoinable){
        this->joinChannel = new Channel(threadName);
    } else {
        this->joinChannel = nullptr;
    }


#ifdef USER_PROGRAM
    this->fdTable = new Table<OpenFile*>;
    space    = nullptr;
#endif
}
...
Thread::~Thread()
{
    ...
    if (this->joinChannel != nullptr) { delete this->joinChannel; }
...
{
    DEBUG('e', "Finishing thread \"%s\"\n", GetName());
    if (isJoinable) {
        this->joinChannel->Send(status);
    }
...
Codes Thread::Join(int* msg) {
    DEBUG('e', "Thread %s join\n", this->GetName());
    if (!this->isJoinable) {
        return NOT_JOINABLE;
    }
...
int
Thread::AddFd(OpenFile* fd) {
    int result = this->fdTable->Add(fd);
    return result == -1 ? -1 : (result+2);
}


OpenFile*
Thread::RemoveFd(OpenFileId fd) {
    return this->fdTable->Remove(fd-2);
}

OpenFile*
Thread::Lookup(OpenFileId fd) {
    return this->fdTable->Get(fd-2);
}

void
Thread::SetAddrSpace(AddressSpace* space) {
    this->space = space;
}
