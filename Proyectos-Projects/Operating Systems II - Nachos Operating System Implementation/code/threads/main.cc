...
main(int argc, char **argv)
{
    ...
    for (argc--, argv++; argc > 0; argc -= argCount, argv += argCount) {
...
#ifdef SWAP
            freeMem = new Coremap(machine->GetNumPhysicalPages());
#else
            freeMem = new Bitmap(machine->GetNumPhysicalPages());
#endif
        ...
        ASSERT(fileSystem->rwLock != nullptr);
            fileSystem->Print();
            printf("\n");
        } else if (!strcmp(*argv, "-ct")) {  // Concurrent test.
            fsProductorConsumidor();
            ...
            PerformanceTest();
        } else if (!strcmp(*argv, "-eft")) {   // extensible test
            ExtensibleTest();
        } else if (!strcmp(*argv, "-pft")) {    // Filesys path test.
            FilesysPathTest();
        }
#endif
    }
