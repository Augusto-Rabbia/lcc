...
public:
    ...
    /// Initialize a `Thread`.
    Thread(const char *debugName, bool willBeJoined = false, int prio = DEFAULT_PRIO);
...
public:
    ...
    int AddFd(OpenFile* fd);
    OpenFile* RemoveFd(OpenFileId fd);
    OpenFile* Lookup(OpenFileId fd);

    void SetAddrSpace(AddressSpace* space);
