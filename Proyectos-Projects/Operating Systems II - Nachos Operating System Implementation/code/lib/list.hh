...
// Para la función de elegir victima en el swap 
template <class Item>
Item
List<Item>::PopN(unsigned nitem)
{
    ListNode *element = first;
    ...
    if (IsEmpty()) {  // If list is empty, put.
        ...
        int i = 0;
        for (ListNode *ptr = element; ; element = element->next, ptr = element) {
            if (element == nullptr) {
                return -1;
            }
            if (i == nitem) {
