...
main(void)
{
    ...
    for (;;) {
        ...
        if (PrepareArguments((line[0] == '&') ? line+1 : line, argv, MAX_ARG_COUNT) == 0) {
        ...
        const SpaceId newProc = Exec2((line[0] == '&') ? line+1 : line, argv);

        // TODO: check for errors when calling `Exec`; this depends on how
        //       errors are reported.
        if (newProc < 0) {
            WriteError("Error: cannot exec the given file.", OUTPUT);
        } else {
            if (line[0] != '&') Join(newProc);
        }
