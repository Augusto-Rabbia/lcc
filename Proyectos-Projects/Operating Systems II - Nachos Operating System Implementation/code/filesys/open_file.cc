...
OpenFile::OpenFile(int sector, const char *name_)
{
    hdr = new FileHeader;
    ...
    name = new char[30];
    strcpy(name, name_);
    this->fileTableEntry = openFilesTable->OpenFile(sector, name);
}

/// Close a Nachos file, de-allocating any in-memory data structures.
OpenFile::~OpenFile()
{
    openFilesTable->CloseFile(this->fileTableEntry->fNode);
    delete hdr;
    delete[] name;
}
...
OpenFile::Read(char *into, unsigned numBytes)
{
    ...
    ASSERT(numBytes > 0);
    ASSERT(this != nullptr);
    ASSERT(this->fileTableEntry != nullptr);
    ASSERT(this->fileTableEntry->rwLock != nullptr);
    this->fileTableEntry->rwLock->AcquireReader();
    int result = ReadAt(into, numBytes, seekPosition);
    seekPosition += result;
    this->fileTableEntry->rwLock->ReleaseReader();
    return result;
}
...
OpenFile::Write(const char *into, unsigned numBytes)
{
    ...
    ASSERT(this->fileTableEntry != nullptr);
    ASSERT(this->fileTableEntry->rwLock != nullptr);
    this->fileTableEntry->rwLock->AcquireWriter();
    int result = WriteAt(into, numBytes, seekPosition);
    seekPosition += result;
    this->fileTableEntry->rwLock->ReleaseWriter();
    return result;
}
...
OpenFile::ReadAt(char *into, unsigned numBytes, unsigned position)
{
    ...
    hdr->FetchFrom(hdr->GetRaw()->headerSector);
...
OpenFile::WriteAt(const char *from, unsigned numBytes, unsigned position)
{
    ...
    DEBUG('w', "fileLength: %u, position: %u, numBytes: %u\n", fileLength, position, numBytes);
   
    if ((int) position-1 >= (int) fileLength) {
        return 0;  // Check request.
    }
    
    
    if (position + numBytes > fileLength) {
        if (!fileSystem->ExtendFile(this, position + numBytes)) {
            // not enough space.
            return 0;
        }
        hdr->FetchFrom(hdr->GetRaw()->headerSector);
