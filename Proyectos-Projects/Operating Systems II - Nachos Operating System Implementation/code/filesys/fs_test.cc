...
static const unsigned FILE_SIZE = CONTENT_SIZE * 15000;

#define CANT_WRITEREAD 150000
...
FileWrite()
{
    ...
    for (unsigned i = 0; i < CANT_WRITEREAD; i += CONTENT_SIZE*13) {
        char buffer[CONTENT_SIZE*13];
        for (unsigned j = 0; j < CONTENT_SIZE*13; j++) {
            buffer[j] = CONTENTS[j % CONTENT_SIZE];
        }
        int numBytes = openFile->Write(buffer, CONTENT_SIZE*13);
        
        if (numBytes < 10) {
            fprintf(stderr, "Perf test: unable to write %s, wrote %d bytes\n", FILE_NAME, numBytes);
            break;
        }
    }
...
FileRead()
{
    ...
    for (unsigned i = 0; i < CANT_WRITEREAD; i += CONTENT_SIZE) {
