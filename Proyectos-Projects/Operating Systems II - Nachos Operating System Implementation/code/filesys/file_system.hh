...
public:
    ...
    bool Create(const char *name, unsigned initialSize = 0, t_directoryType type = T_FILE);

    bool CreateDirectory(const char *name, unsigned initialSize = 0);

    /// Extend a file when written.
    bool
    ExtendFile(OpenFile* file, unsigned newSize);
    ...
    void
    Remove_with_lock(Directory *dir, OpenFile *file, int idx);
