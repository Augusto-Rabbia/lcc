...
public:
    ...
    void Initialize(unsigned sector);
    /// Initialize a file header, including allocating space on disk for the
    /// file data.
    bool Allocate(Bitmap *bitMap, unsigned sector, unsigned fileSize = 0);
    ...
    unsigned SizeToSectors(unsigned numBytes);

    bool Extend(Bitmap* freeMap, unsigned N);

    void AllocateN(Bitmap* freeMap, unsigned N);
