...
bool CountArgsToSave(int address, unsigned *count)
{
    ...
    do {
        bool read = false;
        do {
            read = machine->ReadMem(address + 4 * c, 4, &val);
        } while(!read);
        c++;
    } while (c < MAX_ARG_COUNT && val != 0);
...
SaveArgs(int address)
{
    ...
    for (unsigned i = 0; i < count; i++) {
        ...
        while (!read) {
            read = machine->ReadMem(address + i * 4, 4, &strAddr);
        }
...
WriteArgs(char **args)
{
    ...
    for (unsigned i = 0; i < c; i++) {
        bool wrote = false;
        while (!wrote) {
            wrote = machine->WriteMem(sp + 4 * i, 4, argsAddress[i]);
        }
    }
    bool wrote = false;
    while (!wrote) {
        wrote = machine->WriteMem(sp + 4 * c, 4, 0);  // The last is null.
    }
